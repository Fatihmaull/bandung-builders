...
1. A contract **deployed and verified** on OP Sepolia.
...
| 01:35–02:00 | Block C — End-to-end smoke test (real wallet, real OP Sepolia)     |
...
- [ ] Repo-root `.env` filled:
  - `OP_SEPOLIA_RPC_URL` (private RPC recommended for verification reliability)
  - `PRIVATE_KEY` (testnet only)
  - `OP_ETHERSCAN_API_KEY`
- [ ] Deployer address has at least **0.05 ETH** on OP Sepolia. Top up at <https://console.optimism.io/faucet>.
...
2. With `--broadcast`, actually sends them via `$OP_SEPOLIA_RPC_URL`.
3. With `--verify`, submits each deployed contract's source + constructor args to the Etherscan-compatible API at `api-sepolia-optimism.etherscan.io` (configured in [`foundry.toml`](../../contracts/foundry.toml)).
...
- **"Pending in queue"** — OP Sepolia Etherscan sometimes lags ~30 seconds; the `--watch` flag will keep polling.
...
### A.4. Open OP Sepolia Etherscan, confirm

Visit `https://sepolia-optimism.etherscan.io/address/0xStakingProtocolAddress` (your address). You should see:
...
5. **Environment variables:** add these three:
   ...
   NEXT_PUBLIC_OP_SEPOLIA_RPC_URL=<optional, your private RPC>
   ```
6. **Deploy.**
...
| 2    | Test wallet           | Get OP Sepolia ETH from faucet                        | Wallet balance > 0.01 ETH                                          |
| 3    | Test wallet           | Call `STK.faucet()` via OP Sepolia Etherscan Write Contract → connect wallet → submit | STK balance = 1,000                                |
...
| 9    | Test wallet           | Open OP Sepolia Etherscan → contract address → Events tab            | See `Staked`, `RewardPaid`, `Withdrawn` events                     |
...
- **Earned shows 0 forever** → did the owner call `setRewardRate(...)` after deploy? If not, the rate is still 0. Use OP Sepolia Etherscan Write Contract → connect as owner → call `setRewardRate(0.01 ether)`.
...
- "It is verified at https://sepolia-optimism.etherscan.io/address/____."
...
| Disaster                                                  | Recovery                                                                                                  |
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Forgot to set `rewardRate` after deploy.                   | OP Sepolia Etherscan Write Contract → `setRewardRate(0.01 ether)`. Takes 10 seconds.                                  |
| RPC throttling kills the demo at the worst moment.         | Switch to a private Alchemy/Infura RPC in `NEXT_PUBLIC_OP_SEPOLIA_RPC_URL`, redeploy Vercel (60s).      |
...
| Deployed to wrong chain accidentally.                      | Redeploy on OP Sepolia (Block A), update env, redeploy frontend. Total: 5–10 minutes.                  |
| Contract is verified but reads-tab is empty.                | OP Sepolia Etherscan caches; wait 60 seconds, hard-refresh. If still empty, check that constructor args matched.    |
