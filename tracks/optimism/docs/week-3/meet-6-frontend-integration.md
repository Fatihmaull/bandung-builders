...
- **`chains: [optimismSepolia]`** is the entire chain whitelist. Note the array — multi-chain support would be `[optimismSepolia, base, mainnet]`. Workshop policy: stays one element.
- **`transports`** lets you override the public RPC. We optionally read `NEXT_PUBLIC_OP_SEPOLIA_RPC_URL`. Public OP Sepolia is throttled; a private RPC (Alchemy/Infura/QuickNode) is recommended for any UI that polls.
...
- **Insufficient ETH for gas:** RPC error; surface as "You don't have enough OP Sepolia ETH. Faucet: …".
