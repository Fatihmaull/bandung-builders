# Five Bandung Problem Briefs — DeFi on OP Sepolia

> Companion to [`meet-3-design-thinking.md`](./meet-3-design-thinking.md). Five concrete, locally-grounded problems where a staking-style protocol on OP Sepolia could be the engine.
>
...
### Trust insight
*"Patreon's existence is built on the premise that fans cannot directly pay creators; that premise is false on OP Sepolia because gas is sub-cent. The platform fee is rent that no longer needs to exist, and milestone-conditional pledges are a feature only smart contracts can offer."*
