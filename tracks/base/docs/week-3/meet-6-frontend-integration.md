...
- **`chains: [baseSepolia]`** is the entire chain whitelist. Note the array — multi-chain support would be `[baseSepolia, base, mainnet]`. Workshop policy: stays one element.
- **`transports`** lets you override the public RPC. We optionally read `NEXT_PUBLIC_BASE_SEPOLIA_RPC_URL`. Public Base Sepolia is throttled; a private RPC (Alchemy/Infura/QuickNode) is recommended for any UI that polls.
...
- **Insufficient ETH for gas:** RPC error; surface as "You don't have enough Base Sepolia ETH. Faucet: …".
