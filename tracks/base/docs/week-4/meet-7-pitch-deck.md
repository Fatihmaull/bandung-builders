...
| 4     | Demo                 | 2:00–3:30 | 90 seconds, live, on Base Sepolia          |
...
**Bad hooks:** "We are building a dApp on Base…", "Our team has been working on this for…", "Hello, I'm…".
...
The slot is 90 seconds, and the rule is: it works the first time, in front of judges, on Base Sepolia. Detailed script lives in [Block B](#block-b--the-live-demo-the-make-or-break-35-min) below.
...
```
[Next.js dApp]  --Wagmi-->  [StakingProtocol.sol on Base Sepolia]  --SafeERC20-->  [STK, RWD tokens]
                                          ^
...
2. The L2 (Base Sepolia, ~$0.001/tx, EVM-equivalent).
...
- Wallet pre-connected. Pre-funded with Base Sepolia ETH and STK tokens.
...
| 1:15–1:30 | Open Basescan link to the contract, scroll to "Read Contract" tab | "Everything you just saw — verified, transparent, on Base Sepolia."          |
...
- **Tx slow to confirm:** keep talking. Use the time to explain *what* is happening. "Base Sepolia averages 2-second blocks, sometimes 3 — there we go."
