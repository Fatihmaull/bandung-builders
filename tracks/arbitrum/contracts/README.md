# `contracts/` — Foundry Workspace (Arbitrum Sepolia track)

Solidity sources, tests, and deployment scripts for the Bandung Arbitrum Builders staking protocol.
...
Ensure the track-level `.env` is filled (`ARBITRUM_SEPOLIA_RPC_URL`, `PRIVATE_KEY`, `ARBISCAN_API_KEY`).
...
The `--verify` flag uses the `[etherscan]` block in `foundry.toml` so the contract is verified on Arbiscan in the same run.
