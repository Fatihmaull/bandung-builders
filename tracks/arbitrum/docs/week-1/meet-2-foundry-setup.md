...
3. Use `cast` to call a read function on a verified Arbitrum Sepolia contract from the terminal.
...
| 01:10–01:40 | Block C — `cast` field trip on Arbitrum Sepolia                    |
...
- **`evm_version = "paris"`** — Arbitrum Sepolia supports the Paris/Shanghai hard forks but not all London-prev opcodes. Setting `paris` is the safest cross-OP-Stack choice.

### `[rpc_endpoints]`

```toml
base_sepolia = "${ARBITRUM_SEPOLIA_RPC_URL}"
...
```toml
base_sepolia = { key = "${ARBISCAN_API_KEY}", chain = 421614, url = "https://api-sepolia.arbiscan.io/api" }
```

> "Arbiscan implements the Etherscan v2 API. Same auth header. The same `forge verify-contract` flag works against Arbiscan because of this block. Without this, you'd be passing `--chain-id 421614 --verifier-url ...` on every verify command."
...
## Block C — `cast` field trip on Arbitrum Sepolia (30 min)

The point of this block is to make Arbitrum Sepolia feel **real and reachable from the command line** before they ever write a contract of their own.
...
# Chain ID, confirming we're on Arbitrum Sepolia
...
Pick any verified contract on `sepolia.arbiscan.io` — for example, the canonical Weth9 at `0x4200000000000000000000000000000000000006` (a Base predeploy):

```bash
# Total supply of WETH on Arbitrum Sepolia
cast call \
...
Pick a recent transaction on Arbiscan and grab its raw calldata. Then:
