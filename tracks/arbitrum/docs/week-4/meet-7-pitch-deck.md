...
| 4     | Demo                 | 2:00–3:30 | 90 seconds, live, on Arbitrum Sepolia          |
...
**Bad hooks:** "We are building a dApp on Arbitrum Sepolia…", "Our team has been working on this for…", "Hello, I'm…".
...
The slot is 90 seconds, and the rule is: it works the first time, in front of judges, on Arbitrum Sepolia. Detailed script lives in [Block B](#block-b--the-live-demo-the-make-or-break-35-min) below.
...
```
[Next.js dApp]  --Wagmi-->  [StakingProtocol.sol on Arbitrum Sepolia]  --SafeERC20-->  [STK, RWD tokens]
                                          ^
...
2. The L2 (Arbitrum Sepolia, ~$0.001/tx, EVM-equivalent).
...
- Wallet pre-connected. Pre-funded with Arbitrum Sepolia ETH and STK tokens.
...
| 1:15–1:30 | Open Arbiscan link to the contract, scroll to "Read Contract" tab | "Everything you just saw — verified, transparent, on Arbitrum Sepolia."          |
...
- **Tx slow to confirm:** keep talking. Use the time to explain *what* is happening. "Arbitrum Sepolia averages 2-second blocks, sometimes 3 — there we go."
