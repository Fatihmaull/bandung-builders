...
>
> Cursor reads this automatically when you open the repo. If you use a different agent, paste `docs/system-prompt.md` into its system prompt. **Verify before you start coding.** Ask the agent: 'What network are we targeting?' If it doesn't say 'Base Sepolia, chain ID 84532', your rules aren't loaded."
...
```text
frontend/src/lib/
├── wagmi.ts          <-- single Wagmi config, baseSepolia only
└── contracts.ts      <-- addresses (env-driven) + ABI imports
```

> "Two files, both single-purpose, both load-bearing.
>
> `wagmi.ts` is the chain + connector setup. It's pinned to `[baseSepolia]`. If you ever find yourself wanting to add another chain, that's a workshop policy violation — push back.
>
