...
3. Use `cast` to call a read function on a verified Base Sepolia contract from the terminal.
...
| 01:10–01:40 | Block C — `cast` field trip on Base Sepolia                    |
...
- **`evm_version = "paris"`** — Base Sepolia supports the Paris/Shanghai hard forks but not all London-prev opcodes. Setting `paris` is the safest cross-OP-Stack choice.
...
> "Basescan implements the Etherscan v2 API. Same auth header. The same `forge verify-contract` flag works against Basescan because of this block. Without this, you'd be passing `--chain-id 84532 --verifier-url ...` on every verify command."
...
## Block C — `cast` field trip on Base Sepolia (30 min)

The point of this block is to make Base Sepolia feel **real and reachable from the command line** before they ever write a contract of their own.
...
# Chain ID, confirming we're on Base Sepolia
...
Pick any verified contract on `sepolia.basescan.org` — for example, the canonical Weth9 at `0x4200000000000000000000000000000000000006` (a Base predeploy):

```bash
# Total supply of WETH on Base Sepolia
cast call \
...
Pick a recent transaction on Basescan and grab its raw calldata. Then:
