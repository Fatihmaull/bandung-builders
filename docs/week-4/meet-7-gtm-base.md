# Go-To-Market — Low-Gas Consumer dApps on Base
...
Web2 says: notifications, email digests. Both apply on Base — but the unique Web3 retention loop is *yield accrual*. If their position keeps earning while they're away, the dApp pulls them back to claim.
...
Three legal ways to capture value on Base:
...
6. **"What happens if Base goes down for a week?"** — Your contract is non-custodial, users can withdraw via Etherscan/Basescan or a forked frontend. Be ready.
